...
from ..utils.utils_remision import  (construir_dte_json_notaremision,
...
class NotaRemision(models.Model):
    ...
    factura_entrega = models.ForeignKey(Factura,
    on_delete=models.CASCADE,
    related_name="notas_remision",
    verbose_name="Factura entrega (solo creditos fiscales)",
    default=1,
    blank=False,
    null=True,
    )
    ...
    def save(self, *args, **kwargs):
        self.tipo_factura = '04'  # Por defecto Nota de Remisión
        self.monto = round(self.factura_entrega.total_con_iva, 2)
        if not self.correlativo and self.pk is None:
            prefijos = {
                '04': 'NR',
            }
            prefijo = prefijos.get(self.tipo_factura, 'NR')
            ...
            while True:
                ...
                if not NotaRemision.objects.filter(correlativo=correlativo_temp).exists():
        ...
        try:
            ...
            if not self.firma or self.estado_envio_hacienda in ['pendiente', 'rechazado']:
                construir_dte_json_notaremision(self)
    ...
    def __str__(self):
        return f"Factura {self.correlativo} - {self.cliente.nombre_empresa} - {self.monto}"